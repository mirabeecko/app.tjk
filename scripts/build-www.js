...
const apiBase = process.env.APP_API_BASE || 'https://app.tjkrupka.cz';
...
// <script src="/js/api.js"> i s query verzí (?v=NN)
const apiScriptRe = /<script src="\/js\/api\.js(\?v=[0-9]+)?"><\/script>/;
...
} else if (apiScriptRe.test(html)) {
  html = html.replace(apiScriptRe, inject);
} else {
  // záložní: vložit před </head>
  html = html.replace('</head>', `  <script>window.__API_BASE__ = ${JSON.stringify(apiBase)};</script>\n</head>`);
}
